  ...
  private parseTemperatureCommand(input: string): ParsedCommand {
    ...
    for (const [keyword, kelvin] of sortedKeywords) {
      const patterns = [
        ...
        new RegExp(`(?:set|make|change)\\s+(?:the\\s+)?(?:temperature|temp|color temp)\\s+(?:to\\s+)?${keyword}\\b`, "i"),
    ...
    const kelvinPatterns = [
      /(\d{4,5})\s*k(?:elvin)?/i,
      /(\d{4,5})\s*degrees\s*k(?:elvin)?/i,
    ];
    ...
    for (const pattern of profilePatterns) {
      ...
      if (match) {
        ...
        const matchedProfile = profiles.find((p) => {
          ...
          // Also check tags
          const tagMatch = p.tags?.some((tag) => tag.toLowerCase() === profileNameQuery || tag.toLowerCase().includes(profileNameQuery));
